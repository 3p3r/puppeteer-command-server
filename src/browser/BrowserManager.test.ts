...
describe('BrowserManager', () => {
  ...
  describe('Chrome Path Detection', () => {
    ...
    it('should throw error when chrome not found', async () => {
      ...
      try {
        ...
        expect(error.message).toMatch(/Chrome executable not found|Browser was not found|executablePath/i);
      }
    });
  });
  ...
  describe('Tab Management', () => {
    ...
    it('should throw error when navigating non-existent tab', async () => {
      await expect(browserManager.navigateTab('non-existent-id-12345', 'https://example.com'))
    ...
    it('should throw error when closing non-existent tab', async () => {
      await expect(browserManager.closeTab('non-existent-id-12345'))
        .rejects.toThrow(TabNotFoundError);
    });
  });
  ...
  describe('Browser Actions', () => {
    ...
    it('should evaluate JavaScript', async () => {
      ...
      const result = await browserManager.evaluateScript(tabId, '(function() { return { title: document.title, url: location.href }; })()');
    ...
    it('should handle errors in browser actions', async () => {
      ...
      await expect(browserManager.clickElement(tabId, '.non-existent-element-that-does-not-exist-xyz'))
